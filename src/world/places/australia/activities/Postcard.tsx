...
import { BigButton, KID_NAME, say, sounds, useAlive, wait } from '../../../../sdk'
        ...
        {phase === 'color' ? (
          <motion.div key="color" exit={{ rotateY: 90 }} style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
            <div style={{ flex: 1, minHeight: 0, width: '100%' }}>
              <ColoringPage src={art.colorPip} onFill={setFills} />
            </div>
            {fills >= ENOUGH && (
              <motion.div initial={{ scale: 0 }} animate={{ scale: [1, 1.08, 1] }} transition={{ scale: { repeat: Infinity, duration: 1.2 } }} style={{ position: 'absolute', right: 8, bottom: 8, zIndex: 5 }}>
                <BigButton onClick={() => void send()} ariaLabel="Send">
                  📮 Send!
                </BigButton>
              </motion.div>
            )}
          ...
          >
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', justifyContent: 'space-between' }}>
              ...
              <img src={art.pip} alt="" style={{ height: '48%' }} />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
